...
class MuRP(torch.nn.Module):
    def __init__(self, d, dim):
        ...
        self.rvh.weight.data = 1e-3 * torch.randn(
            (len(d.relations), dim), dtype=torch.double, device="cuda"
        )
        ...
        self.bs = torch.nn.Parameter(
            torch.zeros(
                len(d.entities), dtype=torch.double, requires_grad=True, device="cuda"
            )
        )
    ...
    def forward(self, u_idx, r_idx, v_idx):
        ...
        u_W = u_e * Ru
        u_m = p_exp_map(u_W)
        # TODO move a lot of stuff to CPU
        v_m = p_sum(v, rvh)
        u_m = torch.where(
...
class MuRE(torch.nn.Module):
    def __init__(self, d, dim):
        ...
        self.E.weight.data = 1e-3 * torch.randn(
            (len(d.entities), dim), dtype=torch.double, device="cuda"
        )
        self.Wu = torch.nn.Parameter(
            torch.tensor(
        ...
        self.rv.weight.data = 1e-3 * torch.randn(
            (len(d.relations), dim), dtype=torch.double, device="cuda"
        )
        self.bs = torch.nn.Parameter(
            torch.zeros(
